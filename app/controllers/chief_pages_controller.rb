class ChiefPagesController < ApplicationController
  ...
  def cooking_list
    #TODO
    @sales = Ckfapi::API::Sale.index(detail: true)['sales']
  ...
  def start_cooking
    ...
    if !params[:sale_id].blank?
      @sale = update_next_state_sale(params[:sale_id],"cooking")
      WebsocketRails[:staff].trigger 'next_state_sale',@sale
      respond_to do |format|
        format.js
      end
    elsif !params[:sale_menu_id].blank?
      @sale_menu_item =  update_next_state_sale_menu_item(params[:sale_menu_id],"cooking")
  ...
  def done_cooking
    #TODO
    if !params[:sale_id].blank?
      @sale = update_next_state_sale(params[:sale_id],"done")
      WebsocketRails[:staff].trigger 'next_state_sale',@sale
      respond_to do |format|
        format.js
      end
    elsif !params[:sale_menu_id].blank?
      @sale_menu_item =  update_next_state_sale_menu_item(params[:sale_menu_id],"done")
      WebsocketRails[:staff].trigger 'next_state_sale_menu_item',@sale
      respond_to do |format|
        format.js
      end
